    ...
    <div className="flex">
      <div className={`bg-azul h-screen p-5 pt-8 relative ${open ? "w-72" : "w-20"} duration-300`}>
        ...
        <div className="sidemenu__logo">
          <IoLogoPolymer className="text-5xl max-w-[80px] min-w-[50px]" />
          <div className="leading-[.5]">
            <h1 className={`font-bold text-xl ${!open && "scale-0"}`}>Steel</h1>
            <p className={`text-sm font-light ${!open && "scale-0"}`}>Gestion de Ambientes</p>
          </div>
        </div>
        ...
        <div className="pt-2">
          ...
          {sideMenuOptions.map((option, index) => (
            <li key={option.path}>
              {option.submenu ? (
                <div className="sidemenu__link" onClick={() => toggleSubmenu(index)}>
                  <span>{option.icon}</span>
                  <span className={`${!open && "scale-0"}`}>{option.name}</span>
                  <BsChevronDown />
                </div>
              ) : (
                <Link
                  ...
                  className={`sidemenu__link ${
                    location.pathname.includes(option.path) && "sidemenu__link--active"
                  }`}
                >
                  <span>{option.icon}</span>
                  <span className={`${!open && "scale-0"}`}>{option.name}</span>
